...
**`alternates.languages`:** se centralizó en `buildAlternates(esPath)` (`src/lib/seo/alternates.ts`), que devuelve `{ canonical: esPath, languages: { es: esPath, en: "/en"+esPath, "x-default": esPath } }`. Las dos variantes (es/en) de una misma página declaran el mismo bloque — el hreflang describe el cluster de URLs equivalentes, no depende de cuál lo está sirviendo.
