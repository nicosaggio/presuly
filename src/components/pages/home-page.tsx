...
/**
 * Home pública (marketing). Recibe el locale explícito del route segment que
 * la invoca (`/` = "es", `/en` = "en") en vez de leerlo de la cookie
    ...
    <div className="flex flex-col flex-1">
      ...
      <section className="border-t px-6 py-14">
        <div className="mx-auto max-w-lg text-center space-y-4">
          ...
          <div className="pt-2">
            <LinkButton href={localizedPath(locale, "/pricing")} size="lg">
      ...
      <footer className="flex flex-wrap items-center justify-center gap-4 px-6 py-6 text-xs text-muted-foreground border-t">
        ...
        <Link href={localizedPath(locale, "/terms")} className="hover:text-foreground">
          {dict.common.terms}
        </Link>
        <Link href={localizedPath(locale, "/privacy")} className="hover:text-foreground">
          {dict.common.privacy}
        </Link>
        <Link href={localizedPath(locale, "/refunds")} className="hover:text-foreground">
          {dict.common.refunds}
        </Link>
      </footer>
    </div>
